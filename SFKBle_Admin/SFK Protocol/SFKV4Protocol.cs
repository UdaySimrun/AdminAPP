...
{
    public class SFKV4Protocol : IProtocol
    {
        ...
        public byte[] PrepareCustomerPassword(string sPassword)
        {
            ...
            try
            {
                ...
                byteArrayofReg06[0] = 0xcc;
                byteArrayofReg06[1] = 0xcd;
        ...
        public byte[] PrepareCustomerPassword_Reset(string soldPassword, string sPassword)
        {
            ...
            {
                //start bit
                byteArrayofReg07[0] = 0xcc;
                byteArrayofReg07[1] = 0xcd;
        ...
        public byte[] PrepareSerialNumber(string SerialNumber)
        {
            ...
            {
                //start bit
                byteArrayofRegA2[0] = 0xcc;
                byteArrayofRegA2[1] = 0xcd;
        ...
        public byte[] PrepareDefaultPin09()
        {
            ...
            try
            {
                int iLength = Convert.ToInt32("03", 16) - 2;
                lstByte.Add(0xcc);
                lstByte.Add(0xcd);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32("09", 16))[0]);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32("03", 16))[0]);
                ...
                HexValue = Convert.ToInt32(Day + Month).ToString("X").PadLeft(2, '0') + Convert.ToInt32(Month + Month).ToString("X").PadLeft(2, '0') + Convert.ToInt32(Year + Month).ToString("X").PadLeft(2, '0');

                byte[] bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();

                uint tempSum = Convert.ToUInt32("09", 16);
                for (int i = 0; i < 3; i++)
                {
                    string strHex = Convert.ToInt32(bytes[i]).ToString("X");
                    lstByte.Add(BitConverter.GetBytes(Convert.ToInt32(strHex, 16))[0]);
                    tempSum += Convert.ToUInt32(bytes[i]);
                }
                tempSum += Convert.ToUInt32("03", 16);
                string strCheckSumHex = tempSum.ToString("X").PadLeft(4, '0');
        ...
        public byte[] CreateH2CustomCommand(string register, string length, uint Values = 0, string Command = "")
        {
            ...
            try
            {
                int iLength = Convert.ToInt32(length, 16) - 2;
                List<byte> lstByte = new List<byte>();
                lstByte.Add(0xcc);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32(register, 16))[0]);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32(length, 16))[0]);
                ...
                string HexValue = string.Empty;
                if (Values == 0 && Command == "RTC")
                {
                    DateTime CurrentTime = DateTime.Now;
                    string Date = CurrentTime.Second.ToString("X").PadLeft(2, '0') + CurrentTime.Minute.ToString("X").PadLeft(2, '0') + CurrentTime.Hour.ToString("X").PadLeft(2, '0') + "" + CurrentTime.Day.ToString("X").PadLeft(2, '0') + CurrentTime.Month.ToString("X").PadLeft(2, '0') + CurrentTime.Year.ToString("X").PadLeft(4, '0');
                    HexValue = "00" + Date;
                }
                else
                {
                    HexValue = Values.ToString("X").PadLeft((iLength * 2), '0');
                }
                byte[] bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();

                // var HexValue = Values.ToString("X").PadLeft((iLength * 2), '0');
                // byte[] bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();
                ...
                // tempSum += Convert.ToUInt32(register, 16);
                tempSum += Convert.ToUInt32(iLength + 2);
                string strCheckSumHex = tempSum.ToString("X").PadLeft(4, '0');
        ...
        public byte[] CreateBalanceParameterSetting(int value1, int value2, int value3, int value4)
        {
            ...
            {
                lstByte.Add(0xcc);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32("D9", 16))[0]);
                lstByte.Add(BitConverter.GetBytes(Convert.ToUInt32("0A", 16))[0]);

                var HexValue = value1.ToString("X").PadLeft(4, '0');
                byte[] bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();
                lstByte.Add(bytes[0]);
                lstByte.Add(bytes[1]);

                HexValue = value2.ToString("X").PadLeft(4, '0');
                bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();
                lstByte.Add(bytes[0]);
                lstByte.Add(bytes[1]);

                HexValue = value3.ToString("X").PadLeft(4, '0');
                bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();
                lstByte.Add(bytes[0]);
                lstByte.Add(bytes[1]);

                HexValue = value4.ToString("X").PadLeft(4, '0');
                bytes = Enumerable.Range(0, HexValue.Length / 2).Select(i => Convert.ToByte(HexValue.Substring(i * 2, 2), 16)).ToArray();
                lstByte.Add(bytes[0]);
                lstByte.Add(bytes[1]);

                uint tempSum = 10;
                for (int i = 3; i < lstByte.Count; i++)
                {
                    tempSum += Convert.ToUInt32(lstByte[i]);
                }
        ...
        public byte[] PrepareRegister10Bytes(int nChargeValue)
        {
            ...
            try
            {
                ...
                byteArrayofReg10[0] = 0xcc;
                byteArrayofReg10[1] = 0xcd;
        ...
        public byte[] PrepareRegister11Bytes(double nChargeValue)
        {
            ...
            try
            {
                ...
                byteArrayofReg11[0] = 0xcc;
                byteArrayofReg11[1] = 0xcd;
        ...
        public byte[] GetTempratureBytes(string byteRegister, int nTemprature)
        {
            ...
            try
            {
                ...
                byteArrayofTemprature[0] = 0xcc;
                byteArrayofTemprature[1] = 0xcd;
        ...
        public byte[] GetIdleCurrentRegisterBytes(int Current)
        {
            ...
            try
            {
                ...
                byteArrayofReg28[0] = 0xcc;
                byteArrayofReg28[1] = 0xcd;
        ...
        public byte[] PrepareBytesForNewFirmwareMaxMinSOC(int MaxSoc, int MinSoc, string SOCProtection)
        {
            ...
            try
            {
                ...
                byteArrayofRegE4[0] = 0xcc;
                byteArrayofRegE4[1] = 0xcd;
